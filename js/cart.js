...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(`../api/api.php?endpoint=cart&userID=${userId}`)
    ...
    .catch((error) => {
      ...
      cartTableBody.innerHTML =
        '<tr><td colspan="5">Lỗi khi tải giỏ hàng.</td></tr>';
    });
  ...
  function displayCartItems(cartItems) {
    ...
    cartItems.forEach((item) => {
      ...
      row.innerHTML = `
                <td><input type="checkbox" class="select-item" data-cartitemid="${
                  item.cartItemID
                }"></td>
                <td><img src="${item.image || "placeholder.jpg"}" alt="${
        item.title
      }" style="width: 100px;"></td>
                <td>${item.title}</td>
                <td>${item.bookPrice.toLocaleString("vi-VN", {
                  style: "currency",
                  currency: "VND",
                })}</td>
                <td>${item.quantity}</td>
                <td>${subtotal.toLocaleString("vi-VN", {
                  style: "currency",
                  currency: "VND",
                })}</td>
                <td>
                    <button class="btn btn-danger btn-sm delete-item" data-cartitemid="${
                      item.cartItemID
                    }">Xóa</button>
                    <button class="btn btn-success btn-sm checkout-item" data-cartitemid="${
                      item.cartItemID
    ...
    document
      .getElementById("checkout-all-btn")
  ...
  function checkoutItem(cartItemId) {
    ...
    fetch("../api/api.php?endpoint=checkout", {
      ...
      body: JSON.stringify({ cartItemIDs: [cartItemId], userID: userId }),
    })
      .then((response) => response.json())
      .then((data) => {
        alert(
          data.message ||
            (data.error ? "Lỗi: " + data.error : "Thanh toán thành công!")
        );
        // Tải lại giỏ hàng
        fetch(`../api/api.php?endpoint=cart&userID=${userId}`)
          .then((response) => response.json())
  ...
  function checkoutAllItems() {
    ...
    fetch("../api/api.php?endpoint=checkout", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ cartItemIDs: selectedItems, userID: userId }),
    })
      .then((response) => response.json())
      .then((data) => {
        alert(
